...
4단계 체계는 「재난 및 안전관리 기본법」 제38조(위기경보의 발령), 경계값 산정 방식은 강원도 하천 영향예보 연구 선례를 따랐습니다. 경계값 자체는 현재 표본(n=15) 기준 상대치이며, 표본이 확대되면 재산정이 필요합니다.
...
- `schema.sql`에 더 이상 쓰지 않는 `subscriptions` 테이블 정의가 남아있어 정리가 필요합니다(기능 자체는 이미 제거됨).
